...
class PolarBleSdkManager : ObservableObject {
    ...
    var dynamicYScale: ClosedRange<Double> {
           let minRate = heartRateHistory.map(\.rate).min() ?? 70
           let maxRate = heartRateHistory.map(\.rate).max() ?? 100
           return Double(minRate - 5)...Double(maxRate + 5)
    }
    
    func addHeartRateData(rate: Int) {
            let dataPoint = HeartRateData(time: Date(), rate: rate)
            heartRateHistory.append(dataPoint)
            if heartRateHistory.count > 180 {
                heartRateHistory.removeFirst()
            }
        }
        
    func handleNewHeartRate(rate: Int) {
            if isBroadcastListenOn {
                hr = rate
                addHeartRateData(rate: rate)
                appendHeartRateDataToFile(rate: rate)
            } else {
                hr = 0
            }
    }
            ...
            broadcastDisposable = api.startListenForPolarHrBroadcasts(nil)
                ...
                .subscribe{ e in
                    ...
                    case .next(let broadcast):
                        self.hr = Int(broadcast.hr)
                        NSLog("HR BROADCAST \(broadcast.deviceInfo.name) HR:\(broadcast.hr)")
                    }
                }
        } else {
    ...
    func startAppendingHeartRateDataToFile() {
            let fileName = "HeartRateData.txt"
            ...
            do {
                ...
                print("File ready for appending at \(fileURL.path)")
            } catch {
                print("Error opening file for writing: \(error.localizedDescription)")
            }
        }
    
    func appendHeartRateDataToFile(rate: Int) {
            let currentTime = Date().formatted(date: .numeric, time: .complete)
            let entry = "\(currentTime): \(rate) bpm\n"

            if let data = entry.data(using: .utf8) {
                fileHandle?.write(data)
            } else {
                print("Failed to convert heart rate entry to data.")
            }
        }
    
    func stopAppendingHeartRateDataToFile() {
            do {
                try fileHandle?.close()
                fileHandle = nil
                print("File closed.")
            } catch {
                print("Error closing file: \(error.localizedDescription)")
            }
        }
